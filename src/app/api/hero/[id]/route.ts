...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Error al obtener el slide' }, { status: 500 });
  }
}

// PUT - Actualizar un slide (actualización parcial)
export async function PUT(
  ...
  try {
    ...
    // Solo incluir columnas que fueron enviadas (no sobreescribir con null)
    const allowedFields = ['titulo', 'descripcion', 'imagen_desktop', 'imagen_mobile', 'orden', 'activo'];
    ...
    const result = await db.query(
      `UPDATE hero_slides SET ${setParts.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
      values
    );
    ...
    console.error('Error al actualizar hero slide:', error.message);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await db.query('DELETE FROM hero_slides WHERE id = $1 RETURNING *', [id]);
